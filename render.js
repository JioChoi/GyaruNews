...
app.get('/article/:id', async (req, res) => {
	...
	fs.readFile(__dirname + '/src/article.html', 'utf8', async (err, data) => {
		...
		data = data.replaceAll('${url}', 'https://gyarunews.com/article/' + response.rows[0].id);
...
async function createSitemap() {
	try {
		const smStream = new SitemapStream({ hostname: 'https://gyarunews.com/', xlmns: { news: true, image: true } });
		...
		let query = "SELECT id, date FROM gyarunews ORDER BY date DESC";
